                   ...
                   char_t *url, char_t *path, char_t* query)
{
    ...
    fmtAlloc(&cgiPath, FNAMESIZE, T("%s/%s/%s.exe"), websGetDefaultDir(),
             CGI_BIN, cgiName);
#ifndef VXWORKS
    /*
    ...
    {
        gstat_t		sbuf;
        //wprintf(L"[websCgiHandle] cgipath=%s\n",cgiPath);
...
void websCgiGatherOutput (cgiRec *cgip)
{
    ...
    hOut = CreateFile(cgip->stdOut, GENERIC_READ,FILE_SHARE_READ, NULL,OPEN_EXISTING,FILE_ATTRIBUTE_NORMAL, NULL);
    ...
    SetFilePointer(hOut, cgip->fplacemark, NULL,FILE_BEGIN);
    do {
        ReadFile(hOut, cgiBuf, FNAMESIZE,&nRead,NULL);
        if(nRead <=0)
            break;
        cgiBuf[nRead/2]=0;
        //wprintf(L"[websCgiGatherOutput] nRead=%d,cgip->fplacemark=%d,cgip->stdOut=%s,cgiBuf=%s\n",nRead,cgip->fplacemark,cgip->stdOut,cgiBuf);
        //for(i=0;i<1000000000;i++);
        //websWriteBlock(wp, cgiBuf, nRead);
        websWriteBlock(wp, cgiBuf, gstrlen(cgiBuf));
    ...
    for (cid = 0; cid < cgiMax; cid++) {
        if ((cgip = cgiList[cid]) != NULL) {
            ...
            if (websCheckCgiProc(cgip->handle) == 0) {
                ...
                gunlink(cgip->stdOut);
                /*
                if(gunlink(cgip->stdOut)==-1)
                {
                	char_t a[100];
                	_wgetcwd(a,100);
                	wprintf(L"[gunlink]%d,cgip->stdOut=%s,pwd=%s\n",GetLastError(),cgip->stdOut,a);
                }
                if( DeleteFile(cgip->stdIn) ==0){
                	wprintf(L"[DeleteFile]%d\n",GetLastError());
                }

                if( DeleteFile(cgip->stdOut) ==0){
                	wprintf(L"%d\n",GetLastError());
                }
                */
                /*
...
char_t *websGetCgiCommName()
{
     ...
     */
#if 1
    char_t  pname1[MAX_PATH], *tmp,*pname2;

    //pname1 = gtmpnam(NULL, T("cgi"));
    /*
    ggetcwd(pname1,MAX_PATH);
    tmp = pname1;
    tmp+=wcslen(pname1);
    *(tmp++)=L'/';
    */
    //GetTempFileName(T("."),T("cgi"),0,tmp);
    GetTempFileName(T("\\"),T("cgi"),0,pname1);
    //wprintf(L"[websGetCgiCommName] pname1=%s\n",pname1);
...
int write_data(HANDLE hIn,const char_t *fmt,...)
{
    va_list		args;
    char_t		buf[256];
    ...
    va_end(args);
    WriteFile(hIn,buf,2*need_write,&written,NULL) ;
    return written;
}
                      ...
                      char_t *stdIn, char_t *stdOut)
{
    ...
    //wprintf(L"------------envFile=%s\n",envFile);
    gstrcat(cmdLine, T(" "));
    gstrcat(cmdLine, stdIn);//vonnyfly:add stdIn to cmdline -2
    //wprintf(L"------------stdIn=%s\n",stdIn);
    gstrcat(cmdLine, T(" "));
