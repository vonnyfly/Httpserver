...
int socketReady(int sid)
{
    ...
    for (; sid < socketMax; sid++) {
        ...
        if (sp->flags & SOCKET_CONNRESET) {
            ...
            //wprintf(L"[vonnyfly] socketReady return 0\n");
            return 0;
        }
        if (sp->currentEvents & sp->handlerMask) {
            //wprintf(L"[vonnyfly] socketReady return 1\n");
            return 1;
        }
        /*
...
int socketSelect(int sid, int timeout)
{
    ...
    if (nEvents == 0) {
        ...
        //wprintf(L"[vonnyfly] socketSelect return 0\n");
        return 0;
    }
    ...
    //wprintf(L"[vonnyfly] socketSelect return %d\n",nEvents);
    return nEvents;
}
